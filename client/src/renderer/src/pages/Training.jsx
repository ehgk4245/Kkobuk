    ...
    <div className="min-h-full h-full bg-gray-900 text-white p-8 flex flex-col font-sans relative">
      ...
      <header className="mb-8 text-center animate-fade-in-down relative">
        ...
        <p className="text-gray-400 mt-3 text-sm md:text-base max-w-lg mx-auto leading-relaxed">
          더욱 정확한 거북목 감지를 위해 회원님의 평소 자세를 학습합니다.<br/>
      ...
      <main className="flex-1 flex flex-col items-center w-full">
        <div className="relative w-full max-w-3xl aspect-video bg-black rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-gray-800 group">
          ...
          {/* 실제로는 여기에 <video> 태그가 연결됩니다 */}
          <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-600">
            <Camera size={72} strokeWidth={1.5} className="mb-4 opacity-50 block group-hover:scale-110 transition-transform" />
            <span className="text-sm font-medium tracking-widest">WEBCAM PREVIEW</span>
          </div>
          
          {/* 학습 상태 오버레이 인디케이터 */}
          {trainingMode && (
            <div className={`absolute top-6 left-1/2 -translate-x-1/2 px-8 py-3 rounded-full font-extrabold text-sm shadow-xl animate-bounce ${
              trainingMode === 'good' 
                ? 'bg-[#8BC34A] text-white' 
                : 'bg-[#FFC107] text-gray-900'
            }`}>
              {trainingMode === 'good' ? '✅ 바른 자세 촬영 중... (10s)' : '⚠️ 거북목 자세 촬영 중... (10s)'}
            </div>
          )}
        </div>

        {/* 촬영 조작 버튼 (Capture Controls) */}
        <div className="mt-12 flex gap-6 w-full max-w-2xl px-4">
          <button
            onClick={() => { setTrainingMode('good'); setCapturedGood(true); }}
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <div className="font-extrabold text-white text-lg">
                  바른 자세 촬영 {capturedGood && <span className="text-[#8BC34A] text-sm ml-1">✓ 완료</span>}
                </div>
                <div className="text-xs text-gray-400 mt-1.5 font-medium">허리를 펴고 모니터 정면을<br/>바라보세요</div>
              </div>
            </div>
          </button>

          <button
            onClick={() => { setTrainingMode('bad'); setCapturedBad(true); }}
            disabled={isTraining || isComplete}
            className={`flex-1 bg-gray-800 hover:bg-gray-700 rounded-3xl p-6 transition-all duration-300 transform hover:-translate-y-1 shadow-md border disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 ${
          ...
          >
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <div className="font-extrabold text-white text-lg">
                  거북목 자세 촬영 {capturedBad && <span className="text-[#FFC107] text-sm ml-1">✓ 완료</span>}
                </div>
                <div className="text-xs text-gray-400 mt-1.5 font-medium">평소 작업할 때의<br/>모니터에 집중한 굽은 자세</div>
              </div>
            </div>
          </button>
        </div>
