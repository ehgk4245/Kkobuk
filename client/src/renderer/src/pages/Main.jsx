...
import { Settings, BarChart2, Camera, Play, Square, Pause, Maximize, PictureInPicture2 } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

export default function Main() {
  const navigate = useNavigate();
  // 실제 서비스에서는 전역 상태 또는 웹캠 분석 결과로 연동됩니다.
  const [isGoodPosture, setIsGoodPosture] = useState(true);
  const [isTracking, setIsTracking] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [isMiniMode, setIsMiniMode] = useState(false);

  const handleStop = () => { setIsTracking(false); setIsPaused(false); };
  ...
  if (isMiniMode) {
    return (
      <div className="h-full w-full bg-gray-900 flex flex-col items-center justify-center p-5 select-none transition-colors duration-300 relative z-10" style={{ WebkitAppRegion: 'drag' }}>
        {/* 미니 위젯 뷰 */}
        <div className={`relative w-full aspect-square bg-black rounded-[2rem] overflow-hidden border-4 mb-6 shadow-2xl transition-colors duration-500 ${
          !isTracking
            ? 'border-gray-800'
            : isPaused
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-700">
            <Camera size={36} className={`mb-2 transition-transform ${isTracking ? 'opacity-40' : 'opacity-20'}`} />
          </div>
          {isTracking && (
            <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 w-[85%] py-2 text-center rounded-full font-extrabold shadow-lg text-sm transition-colors ${
              isPaused
        ...
        <div className="flex gap-3 mt-auto" style={{ WebkitAppRegion: 'no-drag' }}>
          {!isTracking ? (
            <button onClick={() => setIsTracking(true)} className="p-4 bg-[#8BC34A] hover:bg-[#7CB342] rounded-2xl text-white shadow-lg transition-transform hover:-translate-y-1">
              <Play fill="currentColor" size={20} />
            </button>
          ) : (
            <>
              <button onClick={handleStop} className="p-4 bg-gray-700 border border-gray-600 hover:bg-gray-600 rounded-2xl text-red-400 shadow-md transition-transform hover:-translate-y-1">
          ...
          <button onClick={toggleMiniMode} className="p-4 bg-blue-600 hover:bg-blue-500 rounded-2xl text-white shadow-lg transition-transform hover:-translate-y-1" title="큰 화면 복귀">
            <Maximize size={20} />
          </button>
        </div>
      </div>
    ...
    <div className="min-h-full h-full bg-gray-900 text-gray-100 p-6 flex flex-col font-sans transition-colors duration-300">
      ...
      <main className="flex-1 flex flex-col justify-center items-center w-full max-w-3xl mx-auto py-2">
        ...
        <div className="w-full flex justify-end gap-3 mb-5">
          <button onClick={toggleMiniMode} className="flex items-center gap-2 px-4 py-2.5 bg-gray-800 rounded-2xl shadow-sm hover:bg-gray-700 transition border border-gray-700 hover:border-gray-600" title="위젯 모드 전환">
            <PictureInPicture2 size={18} className="text-blue-400" />
            <span className="text-sm font-bold text-gray-200">위젯 모드</span>
          </button>
          <button onClick={() => navigate('/dashboard')} className="flex items-center gap-2 px-4 py-2.5 bg-gray-800 rounded-2xl shadow-sm hover:bg-gray-700 transition border border-gray-700 hover:border-gray-600" title="통계 대시보드">
            <BarChart2 size={18} className="text-[#8BC34A]" />
            <span className="text-sm font-bold text-gray-200">주간 통계</span>
          </button>
          <button onClick={() => navigate('/settings')} className="flex items-center gap-2 px-4 py-2.5 bg-gray-800 rounded-2xl shadow-sm hover:bg-gray-700 transition border border-gray-700 hover:border-gray-600" title="환경 설정">
        ...
        {/* 웹캠 및 상태 인디케이터 중앙 뷰어 */}
        <div 
          onClick={() => isTracking && setIsGoodPosture(!isGoodPosture)}
          ...
          !isTracking
            ? 'border-gray-800'
            : isPaused
              ? 'border-blue-600 shadow-blue-600/20 cursor-pointer'
              : isGoodPosture
                ? 'border-[#8BC34A] shadow-[#8BC34A]/20 cursor-pointer'
                : 'border-[#FFC107] shadow-[#FFC107]/20 cursor-pointer'
        }`}>
          {/* 웹캠 플레이스홀더 (화면) */}
          <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-700">
            <Camera size={64} strokeWidth={1.5} className={`mb-4 transition-transform ${isTracking ? 'opacity-40 hover:scale-110' : 'opacity-20'}`} />
            <span className={`text-sm font-bold tracking-widest ${isTracking ? 'opacity-40' : 'opacity-20'}`}>
              {isTracking ? 'LIVE WEBCAM PREVIEW' : 'CAMERA OFF'}
            </span>
          </div>

          {/* 상태 오버레이 (캐릭터 & 텍스트) - 측정 중에만 표시 */}
          {isTracking && (
          <div className="absolute bottom-8 left-0 w-full flex flex-col items-center z-10">
            {isPaused ? (
              ...
              <>
                ...
                <div className={`px-8 py-3 rounded-full font-extrabold text-lg shadow-xl backdrop-blur-md transition-colors ${
                  isGoodPosture
                    ? 'bg-[#8BC34A]/90 text-white'
                    : 'bg-[#FFC107]/90 text-gray-900'
                }`}>
        ...
        <div className="mt-8 w-full flex gap-6">
          <div className="flex-1 bg-gray-800 rounded-[2rem] p-6 shadow-md border border-gray-700 transform transition-transform hover:-translate-y-1">
             <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 rounded-full bg-[#8BC34A]/20 flex items-center justify-center text-[#8BC34A] text-lg">🎯</div>
          ...
          <div className="flex-1 bg-gray-800 rounded-[2rem] p-6 shadow-md border border-gray-700 transform transition-transform hover:-translate-y-1">
             ...
             <p className="text-4xl font-extrabold text-gray-100 mt-3 pl-1">3<span className="text-xl text-gray-400 font-bold mx-1">h</span> 45<span className="text-xl text-gray-400 font-bold mx-1">m</span></p>
          </div>
        </div>
      </main>
    </div>
