  ...
  return (
    <div className="min-h-full h-full bg-gray-900 text-gray-100 p-6 flex flex-col font-sans">
      ...
      <header className="flex items-center mb-8 relative">
        ...
        <h1 className="text-2xl font-extrabold text-center flex-1 pr-12 text-white">
          통계 대시보드
        </h1>
      </header>
          ...
          <div className="space-y-5">
            {[
              ...
              <div key={idx} className="flex items-center gap-4">
                <span
                  className={`w-8 font-bold text-sm ${stat.current ? 'text-[#4CAF50]' : 'text-gray-500'}`}
                >
                  {stat.day}
                </span>
                <div className="flex-1 h-3 rounded-full bg-gray-700 flex overflow-hidden">
                  <div
                    style={{ width: `${stat.good}%` }}
                    className="bg-[#8BC34A] h-full rounded-l-full"
                  ></div>
                  <div
                    style={{ width: `${stat.bad}%` }}
                    className="bg-[#FFC107] h-full rounded-r-full"
                  ></div>
                </div>
